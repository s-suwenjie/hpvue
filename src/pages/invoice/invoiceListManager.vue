      ...
      <template #listBody>
        <tr v-for="(item,index) in content" :key="index" :class="[{twinkleBg: item.id==lastData},{InterlacBg:index%2!=0}]">
          ...
          <yhm-manager-td-operate>
            <yhm-manager-td-operate-button :no-click="item.state === '2'" @click="toVoid(item,0)" value="作废" color="#49a9ea"></yhm-manager-td-operate-button>
            <yhm-manager-td-operate-button :no-click="item.state === '0'" @click="toVoid(item,1)" value="作废原因" color="#49a9ea"></yhm-manager-td-operate-button>
          </yhm-manager-td-operate>
        </tr>
      </template>
      <!--数据空提示-->
