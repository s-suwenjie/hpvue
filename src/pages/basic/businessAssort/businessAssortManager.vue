  ...
  <div>
    <yhm-managerpage category="1">
      ...
      <template #navigationTab>
        <router-link class="menuTabDiv menuTabActive" :to="{path:'/home/businessAssortManager'}">业务类型信息</router-link>
      </template>
      <!--操作区-->
      <template #operate>
      ...
      <template #listHead>
        ...
        <yhm-managerth style="width: 250px"  title="类别编码" value="code" ></yhm-managerth>
        <yhm-managerth title="类别名称" value="name" ></yhm-managerth>
        <yhm-managerth style="width: 250px" title="排序号" value="number" ></yhm-managerth>
    ...
    methods:{
      ...
      del(id){
        this.$dialog.confirm({
          ...
          okCallBack: (data) => {
            ...
            this.ajaxJson({
              url: '/Business/delBusinessAssort',
              data: params,
              call: (data) => {
                if (data.type == '0') {
                  this.$dialog.alert({
      ...
      //搜索
       initPageData (initValue) {
         ...
         this.init({
           initValue:initValue,
           url: '/Business/getBusinessAssortManager',
           data:params,
           all:(data) =>{
             //不管是不是初始化都需要执行的代码
           },
