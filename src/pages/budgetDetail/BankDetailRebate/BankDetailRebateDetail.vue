      ...
      <div class="main_left">
        ...
        <div class="left_bottom" ref="leftBottomHeight">
          <div class="bottom_left float size" style="margin-top: 15px;">
            ...
            <div class="fromTheTop bottom_left_test" ref="fromTheTop" style="margin: 30px 0px 23px 0px;">凭证号：</div>
            <div class="bottom_left_test" style="height: 35px;">备注：</div>
...
<script>
  ...
  export default {
    ...
    methods:{
      leftStrip(){
        window.location='/BankDetailRebateDetail?id='+this.leftID+'&information=0'
      },
      rightStrip(){
        window.location='/BankDetailRebateDetail?id='+this.rightID+'&information=0'
      },
      imgSkip(item){
        let imgUrl = '/UploadFile/' + item.tag + '/' + item.storeName
        window.open(imgUrl)
      },
      switcher(item,index){
      ...
      async isRepeatIssueCode(){
        let params = {
          id: this.id,
          issueCode: this.issueCode
        }
        let result = await this.ajaxAsync({
          url: '/Fin/verifyIssueCode',
          data: params,
          loading: "0",
        })
        if(result.type !== 0){
          this.$refs.issueCode.errorEvent("出单号已存在")
          return false
        }
        return true
      },
      async save(){
        ...
        let b = await this.isRepeatIssueCode();
    ...
    created () {
      ...
      this.setQuery2Value('information')
      ...
      this.init({
        url: '/Fin/getBankDetailRebateByID',
        data: params,
        all: (data)=>{
          try {
            this.content = data
