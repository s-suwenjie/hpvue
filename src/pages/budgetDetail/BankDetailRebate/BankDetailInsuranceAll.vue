<template>
  <div>
    <yhm-managerpage @statisticalClick="statisticalClick" :statisticalShow="true" :total-table="true" :totalWidth="true">

      <template #navigation>售后业务&nbsp;&gt;&nbsp;业务回款&nbsp;&gt;&nbsp;售后回款</template>
      <!--操作区-->
      <template #operate>
      ...
      <template #choose>
        <div v-show="choose" class="buttonBody mptZero">
          <yhm-radiofilter @initData="initChoose('type')" title="回款方式" :content="typeList"></yhm-radiofilter>
      ...
      <template #listBody>
        <tr  v-for="(item,index) in content" :key="index" :class="[{InterlacBg:index%2!=0}]">
          ...
          <yhm-manager-td :tip="true" :value="item.otherName" :color='item.subjectID=="008F808E-C79C-4F8B-9D16-986093FAB86A"?"#0b26e8":""' v-if="item.otherName===''" @click="unitClickLeft(item)" ></yhm-manager-td>
          <yhm-manager-td-center :tip="true" :value="item.otherName" :color='item.subjectID=="008F808E-C79C-4F8B-9D16-986093FAB86A"?"#0b26e8":""' v-else @click="unitClickLeft(item)" :menu-list="unitMenu" @rightClick="rightClick(item)" @menuClick="menuClick"></yhm-manager-td-center>
  ...
  export default {
    ...
    methods:{
      ...
      statisticalClick(){
        ...
        let title = ''
        if(this.typeList.value===''){//点击了全部维修
          url = '/allMaintenanceCartogram'
          title = '全部维修费用统计图'
        }else{
          url = '/insuranceCartogram?type='+this.typeList.value
          title = '保险理赔统计图'
        }
        this.$dialog.OpenWindow({
          width: '1300',
