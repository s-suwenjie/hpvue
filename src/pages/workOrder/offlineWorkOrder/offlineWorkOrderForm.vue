    ...
    <div class="f_main main">
      <yhm-formbody>
        ...
        <template #control>
          ...
          <yhm-form-select title="接待人" @click="selectClient" :value="principal" id="principal" tip="value"></yhm-form-select>
          <yhm-form-select title="车辆" @click="plateEvent" :value="vehicle" id="vehicle" tip="value" rule="R0000"></yhm-form-select>
          <yhm-form-drop-down-select title="付款方" width="1" @select="selectUnit" :select-list="personOrUnitList" :selectValue="personal" selectid="personal" :value="payment" id="payment" rule="R0000"></yhm-form-drop-down-select>
          <yhm-form-select title="联系人" @click="selectPerson" :value="contactPerson" id="contactPerson" rule="R0000"></yhm-form-select>
          ...
          <yhm-form-date title="接待日期" :max-year="Number(currentDate.slice(0,4))" :value="workDate" id="workDate" rule="R0000"></yhm-form-date>
          <yhm-form-date title="预计" subtitle="交车时间"  :value="endDate" id="endDate" rule="R0000"></yhm-form-date>
          <yhm-form-text title="编号" v-if="false" :value="code" id="code" no-edit="1"></yhm-form-text>
          ...
          <yhm-form-radio title="状态" width="1" :select-list="stateList.list" :value="state" id="state"></yhm-form-radio>
          <yhm-form-radio title="工单类型" :select-list="categoryList.list" :value="category" @call="accessNumber" id="category"></yhm-form-radio>
          <yhm-form-radio title="业务来源" :select-list="sourceList.list" :value="source" id="source"></yhm-form-radio>
          <yhm-form-radio title="作业分类" width="1" :select-list="jobtypeList" :value="jobtype" :no-edit="jobtypeNoEdit" @call="jobTypeChange(jobtype)" id="jobtype" rule="R0000"></yhm-form-radio>
...
<!--          &lt;!&ndash;<yhm-form-date title="勘察日期"  :max="currentDate" :max-year="Number(currentDate.slice(0,4))" :value="workDate" id="workDate" rule="R0000"></yhm-form-date>&ndash;&gt;-->
<!--          <yhm-form-text title="定损金额" tip="money" before-icon="rmb" placeholder="请先添加定损单详情" no-edit="1" @input="moneyChange" :value="money" id="money"></yhm-form-text>-->
...
<!--            <yhm-form-td-date width="220" style="text-align: center;" :list="invoiceDetails" listid="invoiceDetails" :value="item" id="insertDate" rule="R0000"></yhm-form-td-date>-->
...
<!--        <template #operate>-->
<!--          <yhm-commonbutton value="添加定损单详情" icon="btnAdd" :is-error="true" :flicker="true" @call="addDetail()" category="three"></yhm-commonbutton>-->
<!--        </template>-->
<!--        <template #listHead>-->
...
<!--            &lt;!&ndash;<yhm-manager-td-look @click="listView(index,item)"></yhm-manager-td-look>&ndash;&gt;-->
...
<!--            <yhm-form-td-delete width="40" :list="list" :value="item" :del-click="true" @click="delServe(index,item,'定损')"></yhm-form-td-delete>-->
<!--          </tr>-->
<!--        </template>-->
<!--        <template #empty>-->
...
<!--      <div class="f_split"></div>-->
      <yhm-form-list-show>
        <template #title>维修项目</template>
        <template #operate>
          <yhm-commonbutton value="添加维修项目" icon="btnAdd" :is-error="true" :flicker="true" @call="addService()" category="three"></yhm-commonbutton>
        </template>
        <template #listHead>
        ...
        <template #customize>
          <div class="aggregateMoney">
            ...
            <div style="margin-right: 60px;">
              ...
              <span style="color: #4BB414;" v-html="priceMoneySegmentation"></span>
            </div>
          </div>
        </template>
      </yhm-form-list-show>
      ...
      <yhm-form-list-show>
        ...
        <template #operate>
          <yhm-commonbutton value="添加维修配件" icon="btnAdd" :is-error="true" :flicker="true" @call="addMaterials()" category="three"></yhm-commonbutton>
        </template>
        <template #listHead>
          <!--<yhm-managerth style="width: 38px" title="查看"></yhm-managerth>-->
        ...
        <template #listBody>
          <tr v-for="(item,index) in fixOrderMaterial" :class="[{InterlacBg:index%2!=0}]" :key="item.id">
            ...
            <yhm-manager-td-money :value="item.price"></yhm-manager-td-money>
            <yhm-manager-td-money :value="item.money"></yhm-manager-td-money>
            <yhm-form-td-select width="120" :list="fixOrderMaterial" listid="fixOrderMaterial" :value="item" id="type" :select-list="typeList"></yhm-form-td-select>
...
<!--          <yhm-managerth style="width: 38px" title="查看"></yhm-managerth>-->
<!--          <yhm-managerth  title="项目名称"></yhm-managerth>-->
<!--          <yhm-managerth style="width: 120px;" title="发生日期"></yhm-managerth>-->
<!--          <yhm-managerth style="width: 120px;" title="变化金额"></yhm-managerth>-->
...
<!--      <div class="f_split"></div>-->
<!--      <yhm-form-list-show>-->
<!--        <template #title>材料表详情</template>-->
<!--        <template #operate>-->
<!--          <yhm-commonbutton value="添加" icon="btnAdd" :is-error="true" :flicker="true" @call="addMaterialsDetail()" category="three"></yhm-commonbutton>-->
<!--        </template>-->
<!--        <template #listHead>-->
<!--          <yhm-managerth style="width: 38px" title="查看"></yhm-managerth>-->
<!--          <yhm-managerth title="项目名称"></yhm-managerth>-->
...
<!--          <yhm-form-text title="合计金额" :value="(Number(price)+Number(amount)).toFixed(2)" id="totalMoney" no-edit="1"></yhm-form-text>-->
<!--          <yhm-form-text title="待结金额" :value="(Number(price)+Number(amount)).toFixed(2)" id="pendingMoney" no-edit="1"></yhm-form-text>-->
...
<!--      </yhm-formbody>-->
        ...
        <template #customize>
          <div class="aggregateMoney">
            ...
            <div style="margin-right: 60px;">
              <span style="font-weight: bold;color: #666666;margin-right: 5px;">总计金额:</span>
              <span style="color: #4BB414;">￥</span>
              <span style="color: #4BB414;" v-html="receivedMoneySegmentation"></span>
            </div>
          </div>
        </template>
      </yhm-form-list-show>
      ...
      <yhm-formbody>
        <template #title>基本信息</template>
        <template #control>
          <yhm-form-text title="项目金额" :value="'全部维修项目变化金额的总和 = '+'项目金额'+' ￥'+price" id="" tip="value" no-edit="1"></yhm-form-text>
          <yhm-form-text title="配件金额" :value="'全部材料详情单价*数量的总和 = '+'配件金额'+' ￥'+amount" id="" tip="value" no-edit="1"></yhm-form-text>
          <yhm-form-text title="合计金额" :value="'项目金额(￥'+Number(price)+')'+ ' + ' +'配件金额(￥'+Number(amount)+') = '+'￥'+(Number(price)+Number(amount)).toFixed(2)" id="" style="white-space:normal" tip="value" no-edit="1"></yhm-form-text>
          <yhm-form-text title="已收金额" :value="revenue+''" id="" style="white-space:normal" tip="value" no-edit="1"></yhm-form-text>
          <yhm-form-text title="未收金额" :value="(Number(price)+Number(amount))-Number(revenue)+''" id="" style="white-space:normal" tip="value" no-edit="1"></yhm-form-text>
        </template>
      </yhm-formbody>
          ...
          call: (data) => {
            if(data){
              ...
              if(data.id!=''){
                ...
                this.accessNumber()
                this.$nextTick(()=>{
                  let fittingsDistance = $('.fittings').offset().top - $(window).scrollTop()
                  let incomeDistance = $('.income').offset().top - $(window).scrollTop()
                  if(this.type=='0'){//查看项目费详情
                    $('html,body').animate({'scrollTop':980},1000)
                  }else if(this.type=='1'){//材料费
                    $('html,body').animate({'scrollTop':Number(fittingsDistance)+16},1188)
                  }else if(this.type=='2'){//合计金额
                    $('html,body').animate({'scrollTop':980},1000)
                  }else if(this.type=='3'){//收入
                    $('html,body').animate({'scrollTop':Number(incomeDistance)+16},1188)
                  }
                })
