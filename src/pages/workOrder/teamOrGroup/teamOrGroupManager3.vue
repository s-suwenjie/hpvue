<template>
  <div>
    <yhm-managerpage>
      <template #navigation>售后业务&nbsp;&gt;&nbsp;工单&nbsp;&gt;&nbsp;钣金装配</template>
      <!--操作区-->
      <template #operate>
      ...
      <template #listHead>
        ...
        <yhm-managerth :show="columnShow[1]" width="110" title="接待日期"></yhm-managerth>
        <yhm-managerth :show="columnShow[2]" title="接待人"></yhm-managerth>
        <yhm-managerth :show="columnShow[3]" width="160" title="工单号"></yhm-managerth>
        <yhm-managerth :show="columnShow[4]" title="维修类型"></yhm-managerth>
        <yhm-managerth :show="columnShow[5]" title="车牌号"></yhm-managerth>
        <yhm-managerth :show="columnShow[6]" title="品牌"></yhm-managerth>
        <yhm-managerth :show="columnShow[7]" width="70" title="车主"></yhm-managerth>
        <yhm-managerth :show="columnShow[8]" width="70" title="联系人"></yhm-managerth>
        <yhm-managerth :show="columnShow[9]" width="70" title="保险公司"></yhm-managerth>
        <yhm-managerth :show="columnShow[10]" title="计划收入金额"></yhm-managerth>
        <yhm-managerth :show="columnShow[11]" title="项目金额"></yhm-managerth>
        <yhm-managerth :show="columnShow[12]" title="材料金额"></yhm-managerth>
        <yhm-managerth :show="columnShow[13]" title="合计金额"></yhm-managerth>
        <yhm-managerth :show="columnShow[14]" width="130" title="状态"></yhm-managerth>
        <yhm-managerth :show="columnShow[15]" width="210" title="操作"></yhm-managerth>
      ...
      <template #listBody>
        <tr :class="[{InterlacBg:index%2!=0}]" v-for="(item,index) in content" :key="index">
          ...
          <yhm-manager-td-center :show="columnShow[3]" :value="item.code"></yhm-manager-td-center>
          <yhm-manager-td-psd    :show="columnShow[4]" :value="item.category" :list="categoryList"></yhm-manager-td-psd>
          ...
          <yhm-manager-td-center :show="columnShow[7]" :value="item.carOwner"></yhm-manager-td-center>
          <yhm-manager-td-center :show="columnShow[8]" :value="item.contactPerson"></yhm-manager-td-center>
          ...
          <yhm-manager-td-operate :show="columnShow[15]" style="display: flex;justify-content: center;margin-left: -16px">
            <yhm-manager-td-operate-button icon="i-btn-applicationSm" @click="lookOver(item)" value="查看当前任务"  :no-click="item.state=='6'||item.state=='7'||item.state=='15'||item.state=='16'?false:true" :color="item.state=='6'||item.state=='7'||item.state=='15'||item.state=='16'?'#8e08e3':'#333'"></yhm-manager-td-operate-button>
            <yhm-manager-td-operate-button icon="i-btn-applicationSm" @click="lookOverAllTask(item)" value="查看历史任务" color="#fd6802"></yhm-manager-td-operate-button>
  ...
  export default {
    ...
    data(){
      return{
        ...
        columnShow:[true,true,true,true,true,true,true,true,true,true,true,true,true,true,true,true],
    ...
    methods:{
      ...
      lookOverAllTask(item){
        this.$dialog.OpenWindow({
          width: '1050',
          height: '800',
          url:  '/teamOrGroupForm?type=5&orderid='+item.id,
          title: '查看全部任务',
          closeCallBack:(data) =>{
      ...
      beginClick(){
        ...
        if(Number(item.orderStr)<=Number(this.indexs)){
          ...
          if(item.nameStr=='选择工序'){
            url = '/workOrderProcessForm?btnShow=1&ownerID='+this.ownerID+'&flowPathID='+this.flowPathID+'&index='+item.orderStr
            title = '选择工序'
            this.skipEvent(url,title)
            return
          }
      ...
      initPageData (initValue) {
        ...
        this.init({
          initValue:initValue,
          url: '/fix/fixOrder/getManagerGroupByTeam',
          data:params,
          all:(data) =>{
            //不管是不是初始化都需要执行的代码
