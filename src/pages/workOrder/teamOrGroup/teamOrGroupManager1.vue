<template>
  <div>
    <yhm-managerpage>
      <template #navigation>售后业务&nbsp;&gt;&nbsp;工单&nbsp;&gt;&nbsp;做底</template>
      <!--操作区-->
      <template #operate>
      ...
      <template #listBody>
        <tr :class="[{InterlacBg:index%2!=0}]" v-for="(item,index) in content" :key="index">
          ...
          <yhm-manager-td-operate :show="columnShow[15]" style="display: flex;justify-content: center;margin-left: -16px">
            <yhm-manager-td-operate-button icon="i-btn-applicationSm" @click="lookOver(item)" value="查看当前任务"  :no-click="item.state=='8'||item.state=='9'?false:true" :color="item.state=='8'||item.state=='9'?'#8e08e3':'#333'"></yhm-manager-td-operate-button>
    ...
    methods:{
      ...
      lookOverAllTask(item){
        this.$dialog.OpenWindow({
          width: '1050',
          height: '800',
          url:  '/teamOrGroupForm?type=1&orderid='+item.id,
          title: '查看全部任务',
          closeCallBack:(data) =>{
