    ...
    methods:{
      ...
      listView(item){
        this.$dialog.OpenWindow({
          width: '1050',
          height: '600',
          url:'/workOrderAddDetailsForm?ownerID='+this.ownerID+'&orderid='+this.orderid+'&id='+item.id+'&aid='+this.applicableModels+'&cid='+this.category+'&flowPathID='+this.flowPathID,
          title:'编辑工单详情',
          closeCallBack:(data) =>{
            if (data) {
              // stockType: "0"
      ...
      addDetail(){
        this.$dialog.OpenWindow({
          width: '1050',
          height: '600',
          url:'/workOrderAddDetailsForm?ownerID='+this.ownerID+ '&id='+'&orderid='+this.orderid +'&flowPathID='+this.flowPathID,
          title:'添加工单详情',
          closeCallBack:(data) =>{
            if (data) {
              this.initData(false)
