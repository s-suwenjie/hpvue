  ...
  <div class="f_main">
    <yhm-formbody>
      ...
      <template #control>
        ...
        <!--        <yhm-form-text title="成本" :value="cost" id="cost" after-icon="rmb" tip="money" no-edit="1"></yhm-form-text>-->
...
<script>
  ...
  import { guid,accMul } from '@/assets/common'
  export default {
    ...
    methods:{
      priceChange(){
        ...
        this.money = accMul(Number(this.quantity),Number(this.price))
      },
      //选择商品型号
      selectModel(){
        if(this.product!=''){
      ...
      selectProduct() {
        this.$dialog.OpenWindow({
          width: 1100,
          height: 603,
          url: '/selectWorkOrderMaterials?category=1',
          title: '选择材料信息',
          closeCallBack: (data) => {
            if (data) {
              this.productid = data.productID
