  ...
  <div>
    <yhm-managerpage category="1" :total-table="true">
      ...
      <template #navigationTab>
        ...
        <router-link class="menuTabDiv menuTabActive" :to="{path:'/home/telephoneSubsidyManager'}">设置话费补助</router-link>
      </template>
      <template #operate>
        <!-- 操作区-->
        ...
        <yhm-radiofilter @initData="initChoose('category')" title="级别" :content="categoryPsd"></yhm-radiofilter>
      </template>
      <template #choose>
      ...
      <template #listHead>
        ...
        <yhm-managerth style="width: 120px;" title="话费补助级别"></yhm-managerth>
    ...
    methods: {
      listView(item){
        this.$dialog.OpenWindow({
          width: 1050,
          height: 420,
          url:'/telephoneSubsidyView?id='+item.id,
          title:'查看话费补助信息',
          closeCallBack:(data) =>{
            if(data){
      ...
      initPageData (initValue) {
        ...
        this.init({
          initValue: initValue,
          url:'/finance/telephoneSubsidy/getManager',
          data: params,
          all: (data) => {
            // 不管是不是初始化都需要执行的代码
          },
