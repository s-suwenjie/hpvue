      ...
      <template #listBody>
        <tr :class="[{twinkleBg: item.id==lastData},{InterlacBg:index%2!=0}]" v-for="(item,index) in content" :key="index">
          ...
          <yhm-manager-td-operate>
            <yhm-manager-td-operate-button @click="register(item)" :no-click="item.state === '2'" value="收款登记" icon="i-btn-applicationSm" color="#49a9ea"></yhm-manager-td-operate-button>
          </yhm-manager-td-operate>
        </tr>
      </template>
      <!--数据空提示-->
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      listView(item){
        if(item.registerCount === '0' && item.state === '0'){
          this.$dialog.OpenWindow({
            width: 1050,
            height: 720,
            url:'/myReceivableForm?id=' + item.id,
            title:'查看应收账款信息',
            closeCallBack:(data) =>{
              if (data) {
                this.initPageData(false)
          ...
          this.$dialog.OpenWindow({
            width: 1050,
            height: 520,
            url:'/myReceivableView?id=' + item.id,
            title:'查看应收账款信息'
          })
      ...
      register(item){
        this.$dialog.OpenWindow({
          width: 1050,
          height: 550,
          url:'/myReceivableRegisterForm?nameID='+item.nameID+'&receivableType='+item.receivableType+'&ownerID='+item.id,
          title:'添加收款登记',
          closeCallBack:(data) =>{
            if (data) {
              this.initPageData(false)
      ...
      initPageData (initValue) {
        ...
        this.init({
          initValue: initValue,
          url: '/Bill/getMyReceivableManager',
          data: params,
          all: (data) => {
            // 不管是不是初始化都需要执行的代码
