      ...
      <template #content>
        <yhm-view-tab-list :customize="true" :pager="false" v-show="tabState[0].select">
          <template #listHead>
            ...
            <yhm-managerth style="width: 90px" title="发票总金额"></yhm-managerth>
      ...
      adoptEvent (item) { //子表通过
        ...
        if(item.id){
          ...
          this.$dialog.OpenWindow({
            ...
            height: 230,
            title: '审批留言',
            url: '/passMessage?id=' + item.id+ '&tableName=40&tableDetailName=41&kind=2',
            closeCallBack: (acc)=>{
              if(acc){
                location.reload()
              }
          ...
          this.ajaxJson({
            ...
            call: (data) => {
              this.$dialog.OpenWindow({
                ...
                height: 230,
                title: '审批留言',
                url: '/passMessage?category='+ data.message +'&id=' + this.id+ '&tableName=40&tableDetailName=41&kind=1',
                closeCallBack: (acc)=>{
                  if(acc){
                    location.reload()
                  }
