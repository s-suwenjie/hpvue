  ...
  <div class="f_main f_main_customize mb16">
    <yhm-select-body>
      ...
      <template #listHead>
        ...
        <yhm-managerth title="手机号" v-show="privateShow"></yhm-managerth>
        <yhm-managerth title="身份证号" v-show="privateShow"></yhm-managerth>
        <yhm-managerth title="账户信息"></yhm-managerth>
        <yhm-managerth width="80" title="常用账户"></yhm-managerth>
      </template>
      ...
      <template #listBody>
        <tr v-for="(item,index) in content" :class="{InterlacBg:index%2!=0}" :key="item.id" @click="selectEvent(item)" @dblclick="dblclickEvent(item)"
          ...
          <yhm-manager-td-center :value="item.phone" format="phone*" v-show="privateShow"></yhm-manager-td-center>
          <yhm-manager-td-center :value="item.idno" format="idno*" v-show="privateShow"></yhm-manager-td-center>
