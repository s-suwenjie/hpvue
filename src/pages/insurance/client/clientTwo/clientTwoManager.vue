...
<!--        <yhm-commonbutton style="margin-left: 30px" value="批量更换负责人" icon="replace"  @call="replace()"></yhm-commonbutton>-->
      ...
      <template #listHead>
        ...
        <yhm-managerth  title="品牌（车型）" value="brand"></yhm-managerth>
...
<!--            <yhm-manager-td-operate-button  @click="replacePrincipal(item.id)" value="" icon="i-replace"  color="#0033FF" ></yhm-manager-td-operate-button>-->
<!--          </yhm-manager-td-operate>-->
        </tr>
      </template>
  ...
  export default {
    ...
    methods:{
        ...
        this.$dialog.OpenWindow({
          ...
          closeCallBack: (data) => {
            if (data) {
              ...
              this.principalID=data[0].id
              this.principal = data[0].name
              this.$dialog.refresh()
              let params = {
                id:arr,
      ...
      initPageData (initValue) {
        ...
        this.init({
          initValue:initValue,
          url: '/Insurance/getPublicClientManager',
          data:params,
          all:(data) =>{
            //不管是不是初始化都需要执行的代码
          },
