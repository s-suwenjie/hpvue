  ...
  <div>
    <div class="center">
      ...
      <div class="flex">
        ...
        <span>车船税金额:</span>
        <div class="cashGiveHPinput">
          <input placeholder="车船税金额" style="width: 100px" type="text" v-model="vehiclesVessels">
          <div class="inputRight">￥</div>
        </div>
        <span>合计金额：</span>
        <div class="cashGiveHPinput">
      ...
      <div class="flex">
        ...
        <input  placeholder="赠送信息,没有可不写" style="width: 180px" type="text" v-model="give">
        <span>联系电话：</span>
        <input  placeholder="联系电话" style="width: 100px" type="text" v-model="contactPhone">
        <span>以当日报价为准 支持转账出单</span>
        <input  placeholder="备注:没有可不写" style="width: 120px" type="text" v-model="remark">
      </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      previewCall(){
        this.previewValue='特斯拉授权钣喷中心姚家园店（'+this.name+'）为您{'+this.plate+'}车报价  商业险金额：'+this.businessMoney+'（'+this.businessCategory+'）交强险金额：'+this.jiaoqiangMoney+'车船税金额'+this.vehiclesVessels+ '合计金额： '+this.totalMoney+' '+this.insuranceUnit+'现金优惠：'+this.discountMoney+' 实收金额：'+this.netReceiptsMoney+' '+this.give+' 联系电话：'+this.contactPhone+' 以当日报价为准 支持转账出单 '+this.remark+' 详情戳https://dllzff.cn/xdyofk15 “回T退订”',
          ...
          this.ajaxJson({
            ...
            call: (data) => {
              if (data.type === 0) {
                ...
                this.details.push(this.vehiclesVessels)
