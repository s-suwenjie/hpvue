...
<!--      </template>-->
      ...
      <template #listBody>
        <tr  v-for="(item,index) in content" :key="index">
          ...
          <yhm-manager-td :value="item.otherName" v-if="item.otherName==''" @click="unitClickLeft(item)" :tip="true"></yhm-manager-td>
          ...
          <yhm-manager-td-operate>
            <yhm-manager-td-operate-button  @click="editBtn(item)" value="分单操作" icon="i-edit"  color="#0033FF"></yhm-manager-td-operate-button>
          </yhm-manager-td-operate>
        </tr>
      </template>
      <!--数据空提示-->
           ...
           for (let i in dataTime){
             ...
             if (count==1){
               this.$dialog.OpenWindow({
                 width: '1050',
                 height: '750',
                 url: '/commissionFeeForm?insuredUnit='+item.otherID +'&useMoney='+item.useMoney+'&ownerID='+item.id,
                 title: '核销应收账款',
                 closeCallBack: (data) => {
               ...
               this.$dialog.confirm({
                 ...
                 cancelCallBack:() => {
                   ...
                   this.$dialog.OpenWindow({
                     width: '1050',
                     height: '750',
                     url: '/commissionFeeForm?insuredUnit='+item.otherID +'&useMoney='+this.countUseMoney+'&index=1',
                     title: '核销应收账款',
                     closeCallBack: (data) => {
