<template>
  <div class="f_main">
    <yhm-view-tab>
      <template #tab>
        <div style="font-size: 16px;" > 直接优惠金额计算</div>
        ...
        <yhm-view-tab-list :customize="true" :pager="true" customize-bg-color="#e2f1f0" v-show="tabState[0].select">
          ...
          <template #listBody>
            <tr v-for="(item,index) in listProfit" :key="index" :class="{InterlacBg:index%2!==0}">
              ...
              <yhm-manager-td-money style="text-align: center;" :value="'-'+item.receivedMoney"></yhm-manager-td-money>
              <yhm-manager-td-center style="color:red;" :value="'='+item.discountMoney"></yhm-manager-td-center>
            </tr>
          </template>
