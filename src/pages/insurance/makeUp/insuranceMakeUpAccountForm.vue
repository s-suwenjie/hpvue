    ...
    <div class="f_main">
      <yhm-formbody>
        ...
        <template #control>
          ...
          <!--            <yhm-form-text title="当前余额" value="0" id="message" no-edit="1"></yhm-form-text>-->
...
<script>
  ...
  export default {
    name: 'insuranceMakeUpAccountForm',
    mixins: [formmixin],
    data () {
      return {
    ...
    methods: {
      ...
      selectaccount () { //选择我方账户信息
        this.$dialog.OpenWindow({
          width: 950,
          height: 690,
          url: '/selectPublicAccount?categoryUnit=0&categoryUnitBefore=1&category=3&categoryInit=2&categoryAccBefore=1',
          title: '选择我方账户信息',
          closeCallBack: (data) => {
            if (data) {
      ...
      selectaccountB () { //选择对方账户信息
        if (this.otherAccountType === '0') {
          ...
          this.$dialog.OpenWindow({
            width: 950,
            height: 640,
            url: '/selectPrivateAccount?commonUse=0&state=0&category=1&categoryUnit=1&categoryUnitBefore=0&personID='+this.otherID,
            title: '选择私人账号',
            closeCallBack: (data) => {
              if(data){
                this.otherAccount = data.publicAccountExplain
      ...
      save () {//保存
        ...
        if (this.validator() && aa) {
          ...
          this.ajaxJson({
            url: '/Fin/vueBankDetailHistorySave',
            data: params,
            loading: '0',
            call: (data) => {
              if (data.type === 0) {
