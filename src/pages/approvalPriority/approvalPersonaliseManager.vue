  ...
  <div>
    <yhm-managerpage category="1">
      ...
      <template #navigationTab>
        ...
        <router-link class="menuTabDiv menuTabActive" :to="{path:'/home/approvalPersonaliseManager'}">审批定制</router-link>
...
<script>
  ...
  export default {
    name: 'approvalPersonaliseManager',
    mixins: [managermixin],
    data () {
      return {
        index:'',
    ...
    methods: {
      add (id) {
        ...
        // 默认设置页面标题为查看信息
        var title = '查看'
        var url = '/approvalPersonaliseForm?id=' + id
        if (!id) { // 当id不存在的时候
          // 设置id为空
          id = ''
          // 设置页面标题为添加信息
          ...
          // 设置页面标记为添加
          isAdd = true
          url = '/approvalPersonaliseForm?id='
        }
      ...
      del(id){
        this.$dialog.confirm({
          ...
          okCallBack: (data) => {
            ...
            this.ajaxJson({
              url: '/Com/approvalPersonaliseDel',
              data: params,
              call: (data) => {
                if (data.type == '0') {
                  this.$dialog.alert({
      ...
      initPageData (initValue) {
        ...
        this.init({
          initValue:initValue,
          url: '/Com/getApprovalPersonaliseManager',
          data:params,
          all:(data) =>{
            //不管是不是初始化都需要执行的代码
          },
