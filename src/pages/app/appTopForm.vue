  ...
  <div class="f_main">
    <yhm-formbody>
      ...
      <template #control>
        ...
        <yhm-form-upload-image title="菜单Logo" tag="TopMenu" discription="建议上传.svg格式" :value="icon" id="icon" rule="#"></yhm-form-upload-image>
  ...
  export default {
    ...
    data(){
      return{
        parentID:'',//父级菜单
        name:'',//菜单名称
        menuType:'0',//菜单类型（0表示顶级菜单，1表示左边的大菜单，2表示详细菜单，3表示权限）
        category:'',//语言类型
        tableName:'',//关联表明
        orderNum:'',//排列序号
        icon:'',//上传图片
    ...
    created () {
       this.setQuery2Value('menuType')
       this.init({
         url: '/SysManager/initAppTopForm?id=',
         all: (data) => {
