      ...
      <div style="position: relative;width: 100%;height: 650px;" :style="{'background':'url('+'http://hp.yhm.hk/UploadFile/OpenInvoice/emptyInvoice'+item.invoiceCategory+'.png)'}">
        ...
        <div class="centername">
          <div v-for="(itm,idx) in item.openInvoiceTrialDetail" class="centerlight">
            <div style="width: 220px;" v-if="item.openInvoiceTrialDetail.length<8">{{itm.product}}</div>
            <div style="width: 100px;" v-if="item.openInvoiceTrialDetail.length<8">{{itm.model}}</div>
            <div style="width: 45px;" v-if="item.openInvoiceTrialDetail.length<8">{{itm.unit}}</div>
            <div style="width: 85px;" v-if="item.openInvoiceTrialDetail.length<8">{{itm.quantity}}</div>
            <div style="width: 110px;text-align: right;" v-if="item.openInvoiceTrialDetail.length<8">{{itm.price}}</div>
            <div style="width: 115px;margin-right: 20px;: 25px;text-align: right;" v-if="item.openInvoiceTrialDetail.length<8">{{itm.totalMoney}}</div>
