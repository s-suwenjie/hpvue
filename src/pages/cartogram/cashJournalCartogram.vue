          ...
          call: (data) => {
            ...
            setTimeout(()=>{
              if(this.month!=='13'){
                ...
                let monthMax = day[money.indexOf(maxSchedule+'')]
                let monthMin = day[money.indexOf(minSchedule+'')]
                if(Number(monthMax)<10){//金额最大值的 日期
                  if((toString(monthMax)).indexOf('0')==-1){
                    monthMax = '0'+monthMax
                  }else{
                    monthMax = monthMax
                  }
                }
                if(Number(monthMin)<10){//金额最小值的 日期
                  if((toString(monthMin)).indexOf('0')==-1){
                    monthMin = '0'+monthMin
                  }else{
                ...
                if(monthMax==undefined&&monthMin!=undefined){
                  this.dateMax = this.year + '年' + month + '月'
                }else if(monthMax!=undefined&&monthMin==undefined){
                  this.dateMin = this.year + '年' + month + '月'
                }else if(monthMax==undefined&&monthMin==undefined){
                  this.dateMax = this.year + '年' + month + '月'
                  this.dateMin = this.year + '年' + month + '月'
                }else{
                  this.dateMax = this.year + '年' + month + '月' + monthMax + '日'
                  this.dateMin = this.year + '年' + month + '月' + monthMin + '日'
                }
                ...
                if(monthMax==undefined&&monthMin!=undefined){
                  this.dateMax = this.year + '年'
                }else if(monthMin==undefined&&monthMax!=undefined){
                  this.dateMin = this.year + '年'
                }else if(monthMax==undefined&&monthMin==undefined){
                  this.dateMax = this.year + '年'
                  this.dateMin = this.year + '年'
                }else{
                  this.dateMax = this.year + '年' + monthMax+ '月'
            ...
            maxSchedule = Math.max.apply(null,money);//最大值
            minSchedule = Math.min.apply(null,money);//最小值
            this.visualMapMin = minSchedule
            this.visualMapMax = maxSchedule
            let num = 0
            let key = 0
            for(let v in money){
