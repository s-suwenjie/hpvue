...
<!--        ----------------------->
      ...
      <div id="BankDetailCommissionCartogram" class="BankDetailCommissionCartogram"></div>
      <div style="width: 100%;height: 24px;"></div>
    </div>
</template>

<script>
  import {zero,formatDate ,tenThousandFormatHtml} from '@/assets/common.js'

  export default {
    name: 'BankDetailCommissionCartogram',
    data(){
      return{
        colors:'#fd6802',//----新加---
    ...
    methods: {
      ...
      rank(type){//排序
        ...
        for(let i in that.moneyList){
          that.allMoney=that.allMoney+that.moneyList[i].money
        }
        //-------------拿上去
        // (that.allMoney.toFixed(2)).toLocaleString()
        let num=that.allMoney.toFixed(2)
        num = parseFloat(num).toFixed(2).toString().split(".");
        num[0] = num[0].replace(new RegExp('(\\d)(?=(\\d{4})+$)','ig'),"$1,");
        // this.splitmore="￥" + num.join(".")
        console.log(num.join("."))
        that.title = that.year + '年'// + '  合计:  ¥'+ num.join(".")
      ...
      drawChart () {
        ...
        let myChart = this.$echarts.init(document.getElementById("BankDetailCommissionCartogram"));
        // 指定图表的配置项和数据
        let option = {
          color: ['#3398DB'],
          tooltip: {
            ...
            formatter(params){
             let label = '日期 ' + params[0].axisValue +'</br>' + '金额 ' + tenThousandFormatHtml(params[0].data+'')
             return label
            }
          },
          // legend: {
          ...
          toolbox: {
            ...
            feature: {
              ...
              saveAsImage: { show: true, name:'保险手续费' + " " + that.downloadTime},
            }
          },
          grid: {
            left: '3%',
        ...
        this.ajaxJson({
          ...
          call: (data) => {
            ...
            setTimeout(()=>{
              ...
              if(day[money.indexOf(maxSchedule)]<10){
                this.dateMax = this.year + '年0' + day[money.indexOf(maxSchedule)]+ '月'
              }else{
                this.dateMax = this.year + '年' + day[money.indexOf(maxSchedule)]+ '月'
              }
              // this.dateMax = this.year + '年' + day[money.indexOf(maxSchedule)]+ '月'
              if(day[money.indexOf(minSchedule)]<10){
                this.dateMin = this.year + '年0' + day[money.indexOf(minSchedule)] + '月'
              }else {
                this.dateMin = this.year + '年' + day[money.indexOf(minSchedule)] + '月'
              }
              // console.log(day[money.indexOf(maxSchedule)])
              // console.log(day[money.indexOf(minSchedule)])
              // this.dateMin = this.year + '年' + day[money.indexOf(minSchedule)] + '月'
    ...
    created(){
      ...
      this.initPageData()
      this.yearTxt=new Date().getFullYear() + ''
    },
    mounted() {
      this.drawChart();
    }
