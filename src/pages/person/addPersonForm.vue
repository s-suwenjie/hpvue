...
<!--        <yhm-form-text title="姓名" :value="name" id="name" ref="name" @repeatverify="repeatVerifyEvent" rule="R0000"></yhm-form-text>-->
        ...
        <yhm-form-text title="手机号码2" :value="phoneReserve" id="phoneReserve" ref="phoneReserve" tip="value" @repeatverify="verifyPersonVuePhone" :rule="phoneReserve==''?'':'R4000'">
          <div v-show="variable" class="formBoxIcon" @click="shareClick(phoneReserve)">
            <span class="icon-uniE9A9" style="font-size: 20px"></span>
          </div>
        </yhm-form-text>
    ...
    methods: {
      ...
      verifyPersonVuePhone(){
        if(this.phoneReserve!==''){
          this.ajaxJson({
            url:"/Basic/verifyPersonVuePhone",
            data:{
              id:this.id,
              phone:this.phoneReserve,
            },
            loading:"0",
            call:(data) =>{
              if(this.phone==this.phoneReserve){
                this.$refs.phoneReserve.errorEvent('两个手机号不能相同')
              }
              if(data.type!=0){
                this.$refs.phoneReserve.errorEvent(data.message)
                this.duplicateAccount(data,data.id)
              }
            },
          })
        }
        ...
        if(this.variableType === '1'){
          if(b){
            let params = {
              ...
              name: this.name,
              phone: this.phone,
              phoneReserve:this.phoneReserve,
              unit: this.unit,
              unitID: this.unitID,
      ...
      initData(){
        this.init({
          ...
          all: (data)=>{
            ...
            this.phoneReserve = data.phoneReserve //手机号码2
